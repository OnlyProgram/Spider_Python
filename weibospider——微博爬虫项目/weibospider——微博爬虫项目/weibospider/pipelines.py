from weibospider.items import TweetsItem,BaseInfoItem,FollowItem,FanItem
import logging
...
class MongoPipeline(object):
    ...
    def process_item(self, item, spider):
        if isinstance(item, BaseInfoItem):   #Python 中的isinstance函数，isinstance是Python中的一个内建函数。是用来判断一个对象的变量类型。
            self.db[self.userinfo].update({'Id': item['Id']}, {'$set': dict(item)}, True)
        if isinstance(item, TweetsItem):
            self.db[self.Tweets].update({'id': item['id']}, {'$set': dict(item)}, True)
        if isinstance(item, FollowItem):
            self.db[self.userinfo].update({'Id': item['Id']}, {'$set': {'FollowsList':item['List']}}, True)
        if isinstance(item, FanItem):
            self.db[self.userinfo].update({'Id': item['Id']}, {'$set': {'FansList':item['List']}}, True)
